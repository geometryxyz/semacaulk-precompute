...
const run = async () => {
    ...
    parser.add_argument('-p', '--port', { action: 'store', type: Number, default: 8000, help: 'The port number to listen on for the proxy to Blyss'})
    ...
    await mainLoop(qtServer, bucket, contract, mutex, startBlockNum, args.main_blocks_per_query, args.finality, args.interval)
}

const genServer = (bucket: Bucket, qtServer: any, mutex: any): typeof Koa => {
    const router = new Router()
    router.get('/:index', async (ctx: any) => {
        let index
        try {
            index = Number(ctx.params.index)
            const max = qtServer.group_size()
            ...
            let data = await bucket.privateRead(index.toString())
            ctx.body = data
            release()
        } catch (e: any) {
            console.error(e)
            ctx.body = 'Server error: ' + e.message
            ctx.status = 500
            return
        }
    })
    ...
    while (true) {
        ...
        if (shouldFetch) {
            ...
            if (logs.length) {
                ...
                const release = await mutex.acquire()
                // Insert the identity commitments into qtServer
                insertIdCommsFromLogs(qtServer, logs, contract.interface)
